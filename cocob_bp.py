...
class COCOB_Backprop(Optimizer):
    ...
    """
    def __init__(self, params, weight_decay=0, alpha=100, lr=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                self.W1.append(p.data.detach().clone())
                self.W_zero.append(p.data.detach().clone().zero_())
                self.W_one.append(p.data.detach().clone().fill_(1))
                self.L.append(p.data.detach().clone().fill_(1))
                self.G.append(p.data.detach().clone().zero_())
                self.Reward.append(p.data.detach().clone().zero_())
                self.Theta.append(p.data.detach().clone().zero_())
